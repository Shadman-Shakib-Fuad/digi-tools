...
const Products = ({ addToCart, setView }) => {
  ...
  return (
    <section className="container" style={{ padding: "80px 0" }}>
      ...
      <p style={{ textAlign: "center", color: "#64748b" }}>Explore powerful tools for your workflow</p>
      <div style={{ display: "flex", justifyContent: "center", gap: "20px", margin: "20px 0" }}>
        <button className="btn btn-primary">Products</button>
        <button className="btn btn-outline" onClick={() => setView("cart")}>Cart</button>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "25px" }}>
        {data.map((item, index) => <ProductCard key={index} item={item} addToCart={addToCart} />)}
      </div>
    </section>
  );
};
